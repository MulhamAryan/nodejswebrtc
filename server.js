...
io.on('connection', function (socket) {
    ...
    socket.on('produce', async (data, callback) => {
        ...
        if (roomName) {
            ...
            if (getStreamer(roomName)) {
                console.log("StreamerID SSSEEENNNNTTTTTT")
                socket.broadcast.to(roomName).emit('set streamerID', getStreamer(roomName));
            }
    ...
    socket.on("streamer ID", () => {
        const id = getId(socket);
        const roomName = getRoomname();
        console.log("Streamer ID: " + id + "   !!!!!HERE!!");

        addStreamer(roomName, id);

        if (getStreamer(roomName)) {
            console.log("StreamerID SSEENNTT");
            socket.broadcast.to(roomName).emit('set streamerID', getStreamer(roomName));
        }
    });

    socket.on("request streamer ID", () => {
        const roomName = getRoomname();
        if (getStreamer(roomName)) {
            console.log("StreamerID SSEENNTT");
            socket.emit('set streamerID', getStreamer(roomName));
        }
    });
    ...
    socket.on('recording data', data => {
        //TODO record data
        const room = Room.getRoom(getRoomname());
        room.pushData(data.chunk);

        if (room.recordingSize() >= 4) {
            // === Make video ===
            // create a writestream to the file recording.mp4
            let wrStream = fs.createWriteStream("../recorded_video/recording_" + socket.id + "_" + data.counter + ".webm");
            // write contents to the file
            wrStream.write(Buffer.concat(room.getRecording()));
            // === Video made ===
            socket.on('recording continue', data => {
                //empty the recording
                room.resetRecording();
            })
        }
...
class Room {
    ...
    pushData(data) {
        this.recording.push(Buffer.from(data));
    }
...
const { PlainTransport } = require('mediasoup/lib/types');
const mediasoupOptions = {
    ...
    webRtcTransport: {
        listenIps: [
            { ip: '192.168.0.176', announcedIp: null }
        ],
        enableUdp: true,
        enableTcp: true,
        preferUdp: true,
...
function addStreamer(roomname, id) {
    const room = Room.getRoom(roomname);
    room.addStreamer(id);
}

function getStreamer(roomname) {
    const room = Room.getRoom(roomname);
    return room.getStreamer();
}
...
async function createRecorderTransport(roomname) {
    let router = null;
    if (roomname) {
        const room = Room.getRoom(roomname);
        router = room.router;
    ...
    const transport = await router.createPlainTransport(mediasoupOptions.PlainTransport);
    console.log('-- create transport room=%s id=%s', roomname, transport.id);
